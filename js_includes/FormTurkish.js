...
jqueryWidget: {
    _init: function () {
        ...
        this.continueMessage = dget(this.options, "continueMessage", "Devam etmek için bu linke tıklayınız");
        ...
        this.obligatoryErrorGenerator =
            dget(this.options, "obligatoryErrorGenerator",
                 function (field) { return "Lütfen bu kısmı doldurunuz"; });
        this.obligatoryCheckboxErrorGenerator =
            dget(this.options, "obligatoryCheckboxErrorGenerator",
                 function (field) { return "Deneye başlamak için lütfen cümlenin solundaki kutuyu tıklayın"; });
        this.obligatoryRadioErrorGenerator =
            dget(this.options, "obligatoryRadioErrorGenerator",
                 function (field) { return "Devam etmek için bir seçeneği seçiniz"; });
            ...
            return function (e) {
                ...
                for (var i = 0; i < checks.length; ++i) {
                    ...
                    rlines.push([["Field name", check.attr('name')],
                                 ["Field value", check.attr('value')],
                                 ["Field value", check.attr('checked') ? t.checkedValue : t.uncheckedValue]]);
                }
                // Added to enable obligatory (select at least one) groups of checkboxes
                //oblig_check_grp = $(dom).find(".obligatory-box-group").each( function(){
                  //  var child_boxes = $( this ).children("input[type=checkbox]:checked")
                    //if (child_boxes.length == 0) {
                      //  alertOrAddError($( this ).attr('name'), t.obligatoryCheckboxErrorGenerator($( this ).attr('name')))
                    //}
                    //$( child_boxes ).each(function(){
                      //  rlines.push([["Field name", this.name],
                        //             ["Field value", check.value],
                          //           ["Field value", check.checked ? t.checkedValue : t.uncheckedValue]]);
